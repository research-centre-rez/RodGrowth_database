...
def extract_base_ps(ps_name: str) -> str:
    ...
    # Hledá počáteční písmena následovaná čísly (např. z "XX01_naklon" udělá "XX01")
...
def highlight_ps_variants(input_path: str, output_path: str) -> None:
    ...
    # Oranžová barva pro zvýraznění variant (s přidaným textem)
    ...
    for col in range(1, sheet.max_column + 1):
        if sheet.cell(row=header_row, column=col).value == DATA_COL_PS:
            ps_col_idx = col
            break
    ...
    # Seskupení řádků podle čistého základu PS
    ...
    # Vyhodnocení skupin a zvýraznění duplicit s textem navíc
    variants_found = 0
